...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "bg-white/[0.028] text-card-foreground flex flex-col gap-5 rounded-xl border border-white/[0.055] py-5 backdrop-blur-xl shadow-[var(--shadow-card)] transition-colors duration-200",
        className
      )}
      {...props}
    />
...
function CardDescription({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("text-muted-foreground/75 text-sm leading-6", className)}
      {...props}
    />
  )
}

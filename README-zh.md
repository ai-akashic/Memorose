...
真正可用的 Agent 记忆系统，必须同时支持事实记忆、过程记忆、跨时间演化，以及 user、agent、app、organization 多层作用域边界。
...
- **领域感知记忆**：覆盖 agent、user、app、organization 四层作用域
...
- **心智模型足够清晰。** `L0-L3` 加上 `Agent/User/App/Organization`，开发者容易讲清，也容易扩展。
...
<table>
  <tr>
    ...
    <td valign="top" width="25%">
      ...
      记忆先在 agent、user、app、organization 各层隔离形成，再按策略向上共享。
    </td>
    <td valign="top" width="25%">
...
- **Support Agent**：结合用户历史、应用级共享知识、组织级知识库
- **Autonomous Agent**：保留过程记忆、分解目标、从完成的里程碑中学习
- **Multimodal Assistant**：从截图、语音、视频上下文中检索记忆
- **多租户 AI 产品**：严格区分 user、app、organization 的记忆边界
...
```
Organization (org_id)          ← 多租户边界
  └─ Application (app_id)     ← 应用隔离
...
|------|------------|------|
| **Organization** | SaaS 多租户边界 | `org: acme-corp` |
| **Application** | 应用级记忆隔离 | `app: coding-assistant` 和 `app: support-bot` |
| **User** | 用户事实、偏好、上下文 | “Dylan 更喜欢 Rust，不喜欢无效会议” |
| **Agent** | 执行轨迹、策略、工具模式 | “API X 在大 payload 下容易失败，应改用 streaming” |

你可以组合这些维度查询，比如：

_“agent-X 在 app-Z 中服务 user-Y 时学到了什么？”_
...
- **Agent / User / App / Organization** 回答的是：这段记忆属于谁、应该为谁服务

这样可以避免把执行经验、个人偏好、产品共享知识、组织知识全部混进一个无差别记忆池。
...
| **App Memory** | 这个产品上下文中有哪些共享知识？ | 共用 workflow、共享术语、应用约定、通用 case | 在单个 `app_id` 内共享，是否贡献取决于策略 |
| **Organization Memory** | 哪些知识应该在组织范围复用？ | 政策、组织术语、公司级知识、跨应用最佳实践 | 在单个 `org_id` 内共享，是否贡献取决于策略 |

### 领域模型速览

| Domain / 领域 | Scope key / 作用域键 | Design purpose / 设计目的 | Native or projected / 原生还是投影 | Typical examples / 典型例子 |
|------|----------|----------|--------------|----------|
| **Agent** | `user_id + app_id + agent_id` | 保留某个 Agent 如何行动 | 原生 | 工具轨迹、执行策略、恢复路径 |
| **User** | `user_id` | 保留用户是谁、喜欢什么 | 原生 | 偏好、身份事实、个人约束 |
| **App** | `org_id + app_id` | 在单个产品内共享复用知识 | 投影 | 共享流程、应用约定、常见解决方案 |
| **Organization** | `org_id` | 在组织级共享更广泛知识 | 投影 | 政策、术语、跨应用最佳实践 |

### 领域边界

- **Agent 记忆** 主要应当是 procedural 的，描述 Agent 如何完成工作，而不是描述用户是谁。
- **User 记忆** 主要应当是 factual 和 preference 的，承载可跨 Agent 复用的稳定个人上下文。
- **App 记忆** 不应该只是 agent 记忆的简单并集。它应该是一个产品内可复用的共享知识层，承载工作流、约定和公共上下文。
- **Organization 记忆** 位于 app 记忆之上，应该保存跨多个 app 仍然成立的组织级知识，而不是某个产品局部的经验。

### 共享模型

Memorose 把 `agent` 和 `user` 视为 **本地原生领域**，新记忆优先在这里形成。  
`app` 和 `organization` 则是 **共享领域**，通过授权后的 projection 进入。

- 新经验应先形成 `agent` 或 `user` 记忆
- `app` 和 `organization` 记忆应来自授权投影，而不是直接混合原始事件
...
- **App Memory**：一个应用里的参与者应当共享什么
- **Organization Memory**：整个组织层面应当复用什么
...
|---|---|---|---|---|
| **L0** | 原始 agent / tool 事件 | 原始用户事件 | 不是主要落地层 | 不是主要落地层 |
| **L1** | 过程记忆 | 事实 / 个人记忆 | 通常由 projection 得到 | 通常由 projection 得到 |
| **L2** | Agent 级反思摘要 | 用户主题与长期洞察 | 应用共享洞察 | 组织共享洞察 |
| **L3** | Agent 计划与里程碑 | 面向用户目标的任务树 | 很少直接作为主要领域 | 很少直接作为主要领域 |

本质上：

- **L0-L3** 解决“记忆有多抽象”
- **Agent/User/App/Organization** 解决“记忆属于谁、谁可以复用”
...
- **Memory Browser**：浏览、搜索、按 user/agent/app 过滤记忆
...
|--------|----------|-------------|
| `POST` | `/v1/users/:uid/apps/:aid/streams/:sid/events` | 写入事件（text/image/audio/video/json） |
| `POST` | `/v1/users/:uid/apps/:aid/streams/:sid/retrieve` | 混合检索，可带跨模态查询输入 |

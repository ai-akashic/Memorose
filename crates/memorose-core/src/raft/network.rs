...
mod tests {
    ...
    async fn test_get_client_rejects_invalid_endpoint() {
        ...
        assert!(
            err.message().contains("Invalid endpoint") || err.message().contains("Connect failed")
        );
    }
    ...
    #[tokio::test]
    async fn test_memorose_raft_server_rejects_invalid_json() -> anyhow::Result<()> {
        use openraft::{Config, Raft};
        use std::sync::Arc;

        let temp_dir = tempfile::tempdir()?;
        let engine = crate::engine::MemoroseEngine::new_with_default_threshold(
            temp_dir.path(),
            1000,
            true,
            true,

...
#[tokio::test]
async fn test_vector_disabled_skips_lancedb_and_preserves_primary_storage() -> Result<()> {
    ...
    assert!(!temp_dir.path().join("lancedb").exists());
    ...
    let unit = MemoryUnit::new(
        ...
        stream_id,
        MemoryType::Factual,
        "Vector disabled primary storage test".into(),
    ...
    let stored = engine
        ...
        .await?
        .expect("memory unit should remain available from RocksDB");
    assert_eq!(stored.content, "Vector disabled primary storage test");

    let vector_hits = engine
        .search_similar(TEST_USER, &embedding, 5, None)
        .await?;
...
async fn test_prune_preserves_l1_referenced_by_l2() -> Result<()> {
    ...
    assert!(engine
        .get_memory_unit(TEST_USER, source_id)
        .await?
        .is_some());
    Ok(())
}
...
async fn test_community_flow() -> Result<()> {
    ...
    let temp_dir = tempdir()?;
    let engine =
        match MemoroseEngine::new_with_default_threshold(temp_dir.path(), 1000, true, true).await {
            Ok(e) => e,
...
impl crate::llm::LLMClient for MockCorrectionLLM {
    ...
    async fn summarize_group(&self, texts: Vec<String>) -> Result<crate::llm::LLMResponse<String>> {
        Ok(crate::llm::LLMResponse {
            data: texts.join("\n"),
            usage: memorose_common::TokenUsage::default(),
        })
...
#[tokio::test]
async fn test_fetch_memory_correction_candidates_supports_multi_clause_history_update() -> Result<()>
{
    let temp_dir = tempdir()?;
    let engine =
        MemoroseEngine::new_with_default_threshold(temp_dir.path(), 1000, true, true).await?;
...
#[tokio::test]
async fn test_fetch_memory_correction_candidates_supports_explicit_contact_transition() -> Result<()>
{
    let temp_dir = tempdir()?;
    let engine =
        MemoroseEngine::new_with_default_threshold(temp_dir.path(), 1000, true, true).await?;
...
#[tokio::test]
async fn test_fetch_memory_correction_candidates_supports_long_mixed_input_with_noise() -> Result<()>
{
    let temp_dir = tempdir()?;
    let engine =
        MemoroseEngine::new_with_default_threshold(temp_dir.path(), 1000, true, true).await?;
...
async fn test_fetch_memory_correction_candidates_supports_long_mixed_forget_and_addition_input(
    ...
    let mut new_unit = MemoryUnit::new(
        ...
        stream_id,
        memorose_common::MemoryType::Factual,
        "Actually, I no longer work at OpenAI. I also speak Japanese. I also love skiing.".into(),
        None,
    );
    new_unit.transaction_time = Utc::now();
...
#[tokio::test]
async fn test_fetch_memory_correction_candidates_supports_according_to_subject_carry() -> Result<()>
{
    let temp_dir = tempdir()?;
    let engine =
        MemoroseEngine::new_with_default_threshold(temp_dir.path(), 1000, true, true).await?;
...
#[tokio::test]
async fn test_apply_memory_correction_actions_tombstones_target_and_links_relation() -> Result<()> {
    let temp_dir = tempdir()?;
    let engine =
        MemoroseEngine::new_with_default_threshold(temp_dir.path(), 1000, true, true).await?;
...
#[tokio::test]
async fn test_fetch_pending_events_marks_malformed_and_missing_user_metadata_failed() -> Result<()>
{
    let temp_dir = tempdir()?;
    let engine =
        MemoroseEngine::new_with_default_threshold(temp_dir.path(), 1000, true, true).await?;
...
async fn test_org_relation_index_is_removed_with_read_view() -> Result<()> {
    ...
    let record = engine
        ...
        .next()
        .expect("expected organization knowledge record");
    let relation_prefix = MemoroseEngine::organization_knowledge_relation_index_prefix(record.id);
    assert!(!engine
        .system_kv()
...
fn test_engine_similarity_policy_and_metric_helpers() {
    ...
    let both =
        MemoroseEngine::organization_similarity_score(&base_record, "cleanup retry", &[1.0, 1.0]);
    let semantic_only =
...
async fn test_engine_l3_task_helpers_and_auto_plan_goal() -> Result<()> {
    ...
    let empty_engine =
        MemoroseEngine::new_with_default_threshold(temp_dir.path().join("empty"), 1000, true, true)
            .await?
            .with_arbitrator(crate::arbitrator::Arbitrator::with_client(Arc::new(
                MockCorrectionLLM {
                    response: "[]".into(),
...
async fn test_get_ready_l3_tasks_filters_blocked_and_missing_dependencies() -> Result<()> {
    ...
    let ready =
        memorose_common::L3Task::new(None, user_id.into(), None, "Ready".into(), "ready".into());
...
mod missing_coverage_tests {
    ...
    use crate::engine::*;
    use crate::fact_extraction::{
        MemoryFactAttribute, MemoryFactChangeType, MemoryFactDescriptor, MemoryFactSubject,
        MemoryFactValueKind, MemoryFactValuePayload,
    };
    ...
    async fn test_search_hybrid_min_score_filtering() -> Result<()> {
        ...
        assert!(
            results.is_empty(),
            "No result should meet a 5.0 min_score threshold"
        );
        Ok(())
    }
    ...
    async fn test_search_text_basic() -> Result<()> {
        ...
        assert!(
            !text_hits.is_empty(),
            "Text search should find the stored unit"
        );
        assert_eq!(text_hits[0].id, unit.id);
        Ok(())
    }

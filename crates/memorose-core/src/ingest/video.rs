...
impl VideoIngestor {
    ...
    pub async fn extract_keyframes(&self, path: &str) -> Result<Vec<Keyframe>> {
        ...
        let entries: Vec<_> =
            tokio::task::spawn_blocking(move || -> Result<Vec<std::fs::DirEntry>> {
        ...
        for (index, entry) in entries.iter().enumerate() {
            ...
            if let Some(model) = model_guard.as_ref() {
                ...
                let should_keep = if let Some(last_emb) = &last_embedding {
                    // Cosine similarity
                    let sum_ab = (embedding.clone() * last_emb.clone())?
                        .sum_all()?
                        .to_scalar::<f32>()?;
                    let norm_a = (embedding.clone() * embedding.clone())?
                        .sum_all()?
                        .sqrt()?
                        .to_scalar::<f32>()?;
                    let norm_b = (last_emb.clone() * last_emb.clone())?
                        .sum_all()?
                        .sqrt()?
                        .to_scalar::<f32>()?;

use anyhow::Result;
use memorose_core::{BackgroundWorker, Event, EventContent, MemoroseEngine};
use std::fs;
...
async fn main() -> Result<()> {
    ...
    println!(
        "✅ L0 Write Complete. Time: {:.2?}, Throughput: {:.0} ops/sec",
        duration,
        n_events as f64 / duration.as_secs_f64()
    );
    ...
    for _ in 0..5 {
        ...
        println!(
            "   Status: L0 Pending: {:<3} | L1 Consolidated: {:<3}",
            pending, l1_count
        );
    ...
    println!(
        "   Final:  L0 Pending: {:<3} | L1 Consolidated: {:<3}",
        engine.fetch_pending_events().await?.len(),
        l1_final
    );
